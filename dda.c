...
/*   Updated: 2025/04/01 14:24:09 by bvelasco         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	dda(float *crds, t_map *map, t_minimap *m_map, t_hitpoint *ht)
{
	...
	while (!detect_colision(dda_data.crds_int[Y] / m_map->size, dda_data.crds_int[X] / m_map->size, map))
	{
		ft_image_pixel_put(m_map->texture, dda_data.crds_int[X], dda_data.crds_int[Y], ~map->floor_color);
		...
		if (costs[X] <= costs[Y])
		{
			...
			if (dda_data.trig[0] != 0)
				dda_data.remains[Y] += fabsf(dists[X] * fabsf((dda_data.trig[2])));
			ht->hit_dir = X;
	...
	if (dda_data.dirs[X] == 1)
		crds[X] = dda_data.crds_int[X] + (dda_data.remains[X] * dda_data.dirs[X]);
	else
		crds[X] = dda_data.crds_int[X] + ((dda_data.remains[X] - 1) * dda_data.dirs[X]);
	if (dda_data.dirs[Y] == 1)
		crds[Y] = dda_data.crds_int[Y] + (dda_data.remains[Y] * dda_data.dirs[Y]);
	else
		crds[Y] = dda_data.crds_int[Y] + ((dda_data.remains[Y] - 1) * dda_data.dirs[Y]);
}
